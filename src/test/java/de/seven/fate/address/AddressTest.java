package de.seven.fate.address;

import de.seven.fate.address.builder.AddressBuilder;
import de.seven.fate.address.model.Address;
import de.seven.fate.dao.BaseEntity;
import org.apache.commons.lang3.SerializationUtils;
import org.junit.Before;
import org.junit.Test;

import javax.persistence.Entity;
import javax.validation.constraints.NotNull;

import static org.junit.Assert.*;

/**
 * Created by Mario on 05.04.2016.
 */

public class AddressTest {

    Address sut;

    AddressBuilder builder = new AddressBuilder();

    @Before
    public void setUp() throws Exception {
        sut = builder.random();
    }

    @Test
    public void testImplementation() throws Exception {
        assertTrue(BaseEntity.class.isAssignableFrom(sut.getClass()));
    }

    @Test
    public void shouldBeEntity() throws Exception {
        assertTrue(Address.class.isAnnotationPresent(Entity.class));
    }

    @Test
    public void emailShouldBeAnnotated() throws Exception {

        assertNotNull(sut.getClass().getDeclaredField("email").getAnnotation(NotNull.class));
    }

    @Test
    public void phoneShouldBeAnnotated() throws Exception {

        assertNotNull(sut.getClass().getDeclaredField("phone").getAnnotation(NotNull.class));
    }

    @Test
    public void mobileShouldBeAnnotated() throws Exception {

        assertNotNull(sut.getClass().getDeclaredField("mobile").getAnnotation(NotNull.class));
    }

    @Test
    public void testEquals() throws Exception {
        Address clone = SerializationUtils.clone(sut);
        assertEquals(sut, clone);
    }

    @Test
    public void testHashCode() throws Exception {
        Address clone = SerializationUtils.clone(sut);
        assertEquals(sut.hashCode(), clone.hashCode());
    }

    @Test
    public void shouldNotEquals() throws Exception {
        Address clone = SerializationUtils.clone(sut);

        clone.setEmail("foo");

        assertFalse(sut.equals(clone));
    }

    @Test
    public void shouldNotHashCode() throws Exception {
        Address clone = SerializationUtils.clone(sut);

        clone.setCity("foo");

        assertFalse(sut.hashCode() == clone.hashCode());
    }
}